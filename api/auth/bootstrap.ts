...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // 5. Create or Resolve Supabase Auth User
    ...
    // 6. Execute Database-level Bootstrap Transaction
    ...
    if (!result || result.status === 'ERROR') {
      const errMsg = result?.message || 'Bootstrap error.';
      const statusCode = errMsg.includes('already registered') ? 409 : 400;
      return res.status(statusCode).json({ error: errMsg });
    }

    // 7. Generate Authenticated Session (server-side only)
