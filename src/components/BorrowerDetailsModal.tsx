...
export const BorrowerDetailsModal: React.FC<BorrowerDetailsModalProps> = ({
  ...
  // Settings-driven Confirmation & Banner state
  ...
  const executeCommitPayment = async () => {
    ...
    if (settings.paymentBannerAlert) {
      setPaymentSuccessToast(`Payment of ₹${parsedAmount.toLocaleString('en-IN')} recorded successfully.`);
    }
  ...
  const handleSavePayment = (e: React.FormEvent) => {
    ...
    if (settings.confirmBeforePayment) {
      setIsConfirmPaymentOpen(true);
      return;
    }

    executeCommitPayment();
  };
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-150">
        <div className="w-full max-w-4xl max-h-[92vh] bg-white rounded-2xl shadow-2xl border border-slate-100 flex flex-col overflow-hidden animate-in zoom-in-95 duration-150">
          ...
          {paymentSuccessToast && (
            <div className="mx-4 sm:mx-6 mt-4 p-3.5 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl flex items-center justify-between text-xs sm:text-sm font-semibold shadow-sm animate-in fade-in duration-200">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-sm space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-3.5 gap-x-6 text-xs sm:text-sm">
                ...
                <div>
                  <span className="text-slate-400 block text-[11px] font-medium">Collection Method</span>
                  <span className="font-semibold text-[#1e293b]">
                ...
                <div>
                  ...
                  <span className="font-semibold text-[#1e293b]">
                    {borrower.endDate ? formatAppDate(borrower.endDate, settings.dateFormat) : '—'}
                  </span>
                </div>
                      ...
                      borrowerPayments.map((p) => (
                        <tr key={p.id} className="hover:bg-slate-50/60 transition-colors">
                          <td className="py-3.5 px-4 sm:px-5 font-semibold text-[#1e293b]">
                            {formatAppDate(p.paymentDate, settings.dateFormat)}
                          </td>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="w-full max-w-sm bg-white rounded-2xl shadow-2xl border border-slate-100 p-6 space-y-4 animate-in zoom-in-95 duration-150">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-base font-bold text-[#1e293b]">Confirm Payment Collection</h4>
                <p className="text-xs text-[#64748b]">Review details before saving</p>
              </div>
            </div>
            ...
            <div className="bg-slate-50 rounded-xl p-3.5 space-y-2 text-xs border border-slate-100">
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-[#1e293b]">{borrower.borrowerName || borrower.name}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-[#1e293b]">{selectedCollectorName}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 font-medium">Date:</span>
                <span className="font-semibold text-[#1e293b]">{formatAppDate(paymentDate, settings.dateFormat)}</span>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              ...
              <button
                type="button"
                onClick={executeCommitPayment}
                disabled={isSubmittingPayment}
                className="px-4 py-2 rounded-xl bg-[#4f46e5] text-white text-xs sm:text-sm font-semibold shadow-sm hover:bg-[#4338ca] active:scale-[0.98] transition-all disabled:opacity-50"
              >
                {isSubmittingPayment ? 'Saving...' : 'Confirm & Save'}
              </button>
            </div>
          </div>
        </div>
